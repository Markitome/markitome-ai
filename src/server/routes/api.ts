...
function getSystemStatus(env: Env): {
  ...
  recallai_bot_provider_configured: boolean;
  ...
  if (!env.RECALLAI_API_KEY) requiredAction.push("Set RECALLAI_API_KEY with npx wrangler secret put RECALLAI_API_KEY to enable meeting bots that join and record.");
  ...
  return {
    ...
    recallai_bot_provider_configured: Boolean(env.RECALLAI_API_KEY),
